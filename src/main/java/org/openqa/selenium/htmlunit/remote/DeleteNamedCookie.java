...
public class DeleteNamedCookie implements HttpHandler {
    private final HtmlUnitDriverServer server;
    private final String sessionId;
    private final String cookieName;

    public DeleteNamedCookie(final HtmlUnitDriverServer server, final String sessionId, final String cookieName) {
        this.server = Require.nonNull("HtmlUnitDriverServer", server);
        this.sessionId = Require.nonNull("Session id", sessionId);
        this.cookieName = Require.nonNull("Cookie name", cookieName);
    }

    @Override
    public HttpResponse execute(final HttpRequest req) throws UncheckedIOException {
        return server.deleteNamedCookie(sessionId, cookieName);
    }
}
